...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `raw-button`                 | native `<button>` markup in consuming Svelte files                                                                                                                                          | `Button` for action chrome; `ButtonBase` for structural controls                           |
...
| `hand-rolled-splitter`       | `cursor: col-resize` or `row-resize` dividers                                                                                                                                               | `SplitResizeHandle`, `CollapsibleSidebar`                                                  |
...
| `hand-rolled-drawer`         | drawer structures plus exact `bottom-dock`/`bottom-panel`/`bottom-tray` tokens (compounds, generic dock names, and kit classes exempt)                                                      | `DetailDrawer`, `BottomDock`                                                               |
...
| Button, ButtonBase                        | `raw-button` flags native button markup; ButtonBase covers structural controls that must keep consumer-owned layout and chrome      |
...
5. **Utilities and theme store**: formatters, `debounce`, `hashColor`
   (note: colors shift from the old consumer `repoColor`), `initTheme`.
