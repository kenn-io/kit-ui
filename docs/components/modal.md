...
| `maxHeight`           | `string`                                                    | `"calc(100svh - 64px)"`            | Cap on the panel's height. svh, not vh: on a phone `100vh` is the height the page gets when the URL bar is hidden, so a panel capped with it can be taller than the screen and the footer is what goes. dvh would resize the dialog as the bar hides and reappears                                                        |
