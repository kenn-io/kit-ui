...
| `SplitResizeHandle`                   | `SplitResizeHandle`                  | `SplitResizeEvent` is axis-neutral: migrate `deltaX` → `delta`, `startX` → `start`, and `currentX` → `current`; raw mouse events become pointer events. Pass `ariaValueMin`, `ariaValueMax`, and `ariaValueNow`. `orientation` defaults to `"horizontal"`; use `"vertical"` for stacked panes                                                               |
