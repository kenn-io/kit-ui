...
| `RangePicker`      | `DateRangePicker` | Renamed (a `RangePicker` alias export exists but is deprecated — import the new name). Decoupled from stores/i18n — all strings are props. The Calendar tab is a month grid now (no day stepping); `previousPeriodLabel`/`nextPeriodLabel` still work as deprecated aliases. `resolveRange()` bounds are inclusive; "Last N days" includes today; periods containing `maxDate` resolve to full bounds (clamp consumer-side) |
