...
Push the current feature branch to origin. Create a PR whose body leads with why Forge needs a shared inline dock distinct from `DetailDrawer`, summarizes both public contracts, lists complete validation, and notes that downstream Forge migration is intentionally out of scope. Every GitHub comment or PR body written by the agent must end with:
