...
**Goal:** Make kit-ui's large form inputs and actions share the approved 36px height.

**Architecture:** Keep the existing `lg` size API. Update the three primitives that own large control geometry, then let `FormField` and `ProviderButton` continue inheriting that geometry through composition. Browser coverage will protect the cross-component relationship and reject both the compact 28px and oversized 40px endpoints.

**Tech Stack:** Svelte 5, scoped CSS, Playwright, Vite+

## Global Constraints

- The shared `lg` form-control height is 36px.
...
- Produces: A 36px large control height shared by `Button`, `TextInput`, `FormField`, and `ProviderButton`.
...
- [x] **Step 3: Implement the 36px shared size**
  ...
  Change `.kit-button--lg`, `.kit-text-input--lg`, and ProviderButton's default `--provider-button-height` fallback to 36px. Preserve all existing padding, typography, state, and public override behavior.
