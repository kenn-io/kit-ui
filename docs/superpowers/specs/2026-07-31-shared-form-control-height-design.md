...
Use 36px as kit-ui's shared `lg` form-control height. `Button` and `TextInput`
will define that size once. `FormField` and `ProviderButton` will continue to
compose those large primitives, so consumers get matching input and action
geometry without local CSS.
