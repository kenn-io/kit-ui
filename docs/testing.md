...
| `refresh-control.spec.ts`      | Width reservation: the age and detail boxes measure the same across the narrowest, widest, and overlong variants, the element after the control does not move, and the detail readout stays right-aligned                                                             |
